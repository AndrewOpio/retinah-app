...
message = (email, msg) =>{
  ...
  add = () =>{
    ...
    if(this.state.name != "" && this.state.email != "" && this.state.location != "" && this.state.contact != "" 
    && this.state.latitude != "" && this.state.longitude != "" && this.state.password != ""){
      if(this.state.password !=""){

        email = this.state.email;
        fetch('https://agencyforadolescents.org/RetinahBackend/RetinahBackend/hospital.php',
        //'http://192.168.64.1/RetinahBackend/hospital.php',
        {
          method : 'post',
          header : {
            'Accept' : 'application/json',
        ...
        .then((responseJson) => {
            ...
            this.message("police@gmail.com", "Hello, welcome to Police");
            Alert.alert(
              "Congs",
...
location = (value) => {
  ...
  return (
      <ScrollView>
        <Text style ={{ marginLeft: 5, fontSize : 18, marginTop : 10, fontStyle : "italic"}}>Name:</Text>
        <TextInput placeholder="name (required)" placeholderColor="#c4c3cb" style={styles.loginFormTextInput}  onChangeText = {(text) => this.setState({name : text})}/>            
        ...
        <Text style ={{ marginLeft: 5, fontSize : 18, marginTop : 10, fontStyle : "italic"}}>Contact:</Text>
        <TextInput placeholder="contact (required)" placeholderColor="#c4c3cb" style={styles.loginFormTextInput}  onChangeText = {(text) => this.setState({contact : text})}/>            
        
          <Text style ={{ marginLeft: 5, fontSize : 18, marginTop : 10, fontStyle : "italic"}}>Latitude:</Text>
          <TextInput placeholder="latitude (required)" placeholderColor="#c4c3cb" style={styles.loginFormTextInput}  onChangeText = {(text) => this.setState({latitude : text})}/>            
          
          <Text style ={{ marginLeft: 5, fontSize : 18, marginTop : 10, fontStyle : "italic"}}>Longitude:</Text>
          <TextInput placeholder="longitude (required)" placeholderColor="#c4c3cb" style={styles.loginFormTextInput}  onChangeText = {(text) => this.setState({longitude : text})}/>            
        
        <Text style ={{ marginLeft: 5, fontSize : 18, marginTop : 10, fontStyle : "italic"}}>District:</Text>
        <View style={styles.loginFormTextInput}>
          <Picker selectedValue = {this.state.location}
            ...
            <TextInput placeholder="password (required)" placeholderColor="#c4c3cb"  style={styles.loginFormTextInput}  onChangeText = {(text) => this.setState({password : text})}/>            
